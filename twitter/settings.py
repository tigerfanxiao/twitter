...
SECRET_KEY = "a%#23om39!nj-26m91uj2rn5tzjp#w*k=3$ootw+9!-u9j8l$)"
...
if TESTING:
    DEFAULT_FILE_STORAGE = 'django.core.files.storage.FileSystemStorage'  # 默认使用 django 自己的

# 当用s3boto3 作为用户上传文件存储时，需要按照你在 AWS 上创建的配置来设置你的 BUCKET_NAME
# 和 REGION_NAME，这个值你可以改成你自己创建的 bucket 的名字和所在的 region
AWS_STORAGE_BUCKET_NAME = 'django-twitter-xiao'
